    ...
    $(function(){
    	 ...
    	 $.ajax({
         	...
         	success:function(data){
         		//alert(JSON.stringify(data.newestSeries));
         ...
         //头像信息
         //alert(result.author.imgUrl)
         if(result.author!=null){
          	if(result.author.imgUrl!=""){
             if(result.author.imgUrl.indexOf('http')>-1){
            	 $("#authorImg").attr("src","${ctx}/common/file/image/"+result.author.imgUrl); 
             }else{
            	 $("#authorImg").attr("src","${ctx}/"+result.author.imgUrl);  
             }
            }else{
