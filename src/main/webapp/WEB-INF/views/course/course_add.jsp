...
<script id="formEditSectionTitle" type="text/x-dot-template">
    <div class="form-edit-title form-horizontal">
        <div class="control-group">
            <label class="control-label">第<span class="index">{{?it.chapter}}{{=it.chapter.num}}</span>章{{??it.lecture}}{{=it.lecture.num}}</span>
                节{{?}}</label>

            <div class="controls">
                <input type="text" maxlength="20" class="input-block-level" placeholder="请输入课程标题"
                       value="{{=(it.chapter ? it.chapter.title : it.lecture.title) || ''}}"/>
...
<script id="sectionBarTemplate" type="text/x-dot-template">
    ...
    <div class="sortable-bar">
			...
			{{?param.type}}<label class="label">{{?param.isElective!=null &&
                    param.isElective=='0'}}选修{{??}}必修{{?}}</label>{{?}}
			</span>
...
<script id="lectureContentTemplate" type="text/x-dot-template">
    ...
    <div class="lecture-content hide">
        ...
        <div class="bd">
            <a class="btn-type{{?param.type != 'none' && param.type != 'video'}} disabled{{?}}" href="#video"><i
                    class="icon-video-lg"></i><h5>视频</h5></a>
            <a class="btn-type{{?param.type != 'none' && param.type != 'audio'}} disabled{{?}}" href="#audio"><i
                    class="icon-audio-lg"></i><h5>音频</h5></a>
            <a class="btn-type{{?param.type != 'none' && param.type != 'doc'}} disabled{{?}}" href="#doc"><i
                    class="icon-doc-lg"></i><h5>文档</h5></a>
            <a class="btn-type{{?param.type != 'none' && param.type != 'ppt'}} disabled{{?}}" href="#ppt"><i
                    class="icon-ppt-lg"></i><h5>幻灯片</h5></a>
            <a class="btn-type{{?param.type != 'none' && param.type != 'img'}} disabled{{?}}" href="#img"><i
                    class="icon-img-lg"></i><h5>图片</h5></a>
            <a class="btn-type{{?param.type != 'none' && param.type != 'exam'}} disabled{{?}}" href="#exam"><i
                    class="icon-exam-lg"></i><h5>测试</h5></a>
            <a class="btn-type{{?param.type != 'none' && param.type != 'task'}} disabled{{?}}" href="#task"><i
                    class="icon-task-lg"></i><h5>作业</h5></a>
            <!--
            ...
            <form id="formBasicInfo" method="post" action="{{=it.action || '##'}}">
                <div class="section">
                    <fieldset>
                        ...
                        <input type="text" id="courseTitle" name="courseTitle" required minlength="6"
                               class="input-block-level" value="{{=it.courseTit || ''}}"/>
                        <label for="subTitle">副标题</label>
                        <textarea name="subTitle" id="subTitle" required minlength="12" class="input-block-level"
                                  rows="3">{{=it.subTit || ''}}</textarea>
                        ...
                        <div class="btn-group btns-radio" data-toggle="buttons-radio">
                            <button class="btn btn-large{{?it.sectionOrder==null || it.sectionOrder}} active{{?}}"
                                    id="true" type="button">顺序学习
                            </button>
                            <button class="btn btn-large{{?it.sectionOrder!=null && !it.sectionOrder}} active{{?}}"
                                    id="false" type="button">无序学习
                            </button>
                        </div>
                        <label>关键词</label>
                    ...
                    <ul class="nav nav-pills courseType">
                        ...
                        <li
                        {{?type.id == it.courseType}} class="active"{{?}}><a href="#">{{=type.title}}</a><input
                            type='hidden' value='{{=type.id}}'></li>
                        {{~}}
                    </ul>
                </div>
...
<script id="detailInfoTemplate" type="text/x-dot-template">
    ...
    <div class="page-body detail-content">
        <form id="formDetailInfo" method="post" action="{{=it.action || '##'}}">
            <div class="section">
                ...
                <textarea id="courseAbstract" name="courseAbstract" required minlength="20" class="input-block-level"
                          rows="4">{{=it.courseAbstract || ''}}</textarea>
                ...
                <textarea id="authorDescrip" name="authorDescrip" class="input-block-level" rows="4">{{=it.authorDescrip
                    || ''}}</textarea>
                <label for="learnObjectives">学习目标</label>
                <input type="hidden" id="learnObjectives" name="learnObjectives"
                       value="{{= it.learnObjectives || '' }}"/>
                ...
                <input type="hidden" id="courseRequirements" name="courseRequirements"
                       value="{{= it.courseRequirements || '' }}"/>
                <ul class="list_alert nav">
    ...
    <div class="formAdd form-inline control-group"><input type="text" placeholder="请填写{{=param}}"/>
        <button class="btn btn-large btn-primary" type="button">添加</button>
    </div>
    #}}
    {{##def.item:param:
    ...
    <div class="page-body promotion-content">
        <form id="formPromotion" method="post" action="{{=it.action || '##'}}">
            <div class="section">
                ...
                <!--图片预览-->
                <div class="courseCover"><img id="imgshow" name="imgshow" style="width: 300px;height:200px;"
                                              src="{{=it.coverUrl || '${ctx}/resources/images/zht-main-img.jpg'}}"
                                              alt=""/></div>
            </div>
            <div class="section">
            ...
            <div class="courseSkins">
                ...
                <ul class="nav courseSkinList clearfix">
                    ...
                    <li
                    {{?skin.title == it.courseSkin.title}} class="active"{{?}}><a href="#"><img src="{{=skin.imgUrl}}"
                                                                                                alt="{{=skin.title}}"/><i
                        class="icon-right"></i></a><h5>{{=skin.title}}</h5></li>
                    {{~}}
                </ul>
            </div>
        ...
        <form id="formAccessRight" method="post" action="{{=it.action || '#accessRight'}}">
            <div class="section">
                ...
                <ul class="nav nav-pills">
                    <li
                    {{?it.permission == 'open' || it.permission == ''}} class="active"{{?}}><a data-toggle="tab"
                                                                                               href="#open">公开</a></li>
                    <li
                    {{?it.permission == 'encrypt'}} class="active"{{?}}><a data-toggle="tab" href="#encrypt">加密</a></li>
                </ul>
                <div class="tab-content">
                    ...
                    <div class="tab-pane{{?it.permission == 'encrypt'}} active{{?}}" id="encrypt">
                        <label class="radio" for="authorized"><input type="radio" onclick="removePass()"
                                                                     value="authorized" {{?it.encryptType ==
                            'authorized' || it.encryptType == ''}}checked{{?}} name="encryptType" id="authorized"
                            /><span class="labelTxt">授权组织备课</span>前往 <a href="#kinguser" onClick="urlRouter()">授权管理</a>
                            本课程的用户列表</label>
                        <label class="radio" id="passRadio" for="passwordProtect"><input type="radio"
                                                                                         value="passwordProtect"
                            {{?it.encryptType == 'passwordProtect'}}checked{{?}} name="encryptType" id="passwordProtect"
                            /><span class="labelTxt">密码保护</span><input type="password" id="coursePwd" name="coursePwd"
                            {{?it.encryptType !='passwordProtect'}}disabled{{?}} placeholder="请填写课程访问的密码"
                            value="{{=it.coursePwd || ''}}" /></label>
                    </div>
                </div>
            </div>
                    ...
                    <td class="tdTit">
                        <div class="pr">
                            <div class="state-dragable"><span class="icon-bar"></span><span
                                    class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span><span
                                    class="icon-bar"></span></div>
                            <img src="{{=user.imgUrl || 'images/temp-face36.jpg'}}" alt="">{{=user.name}}（{{=user.mail}}），{{=user.org}}
                            {{=user.department}}
                        </div>
                    </td>
...
<script id="listUserKinguserTemplate" type="text/x-dot-template">
    <tr data-fdid="{{=it.id}}">
        <td class="tdTit">
            <div class="pr">
                <div class="state-dragable"><span class="icon-bar"></span><span class="icon-bar"></span><span
        ...
        <form id="formMedia" method="post" class="form-horizontal" action="{{=it.action || '##'}}">
            <div class="section">
                    ...
                    <div class="controls">
                        ...
                        <div class="btn-group btns-radio" data-toggle="buttons-radio">
                            <button class="btn btn-large{{?it.isElective=='1' || it.isElective==null}} active{{?}}"
                                    id="obligatory" type="button">必修
                            </button>
                            <button class="btn btn-large{{?it.isElective=='0'}} active{{?}}" id="elective"
                                    type="button">选修
                            </button>
                        </div>
                    </div>
                ...
                <div class="control-group">
                    ...
                    <div class="controls"><input value="{{=it.pageTitle || ''}}" id="lectureName" required
                                                 class="input-xlarge" name="lectureName" type="text"/></div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="learnTime">学习时长</label>

                    <div class="controls"><input value="{{=it.learnTime || ''}}" required placeholder="请认真填写该章节的建议学习时长"
                                                 id="learnTime" class="input-xlarge" name="learnTime" type="text"/>
                    </div>
                </div>
                <div class="control-group">
                    ...
                    <div class="controls">
                        <textarea placeholder="请认真填写该章节的描述信息" rows="4" required minlength="20" class="input-xxlarge"
                                  id="sectionsIntro" name="sectionsIntro">{{=it.sectionsIntro || ''}}</textarea>
                    </div>
                </div>
            </div>
            <div class="mediaList">
                ...
                <ul class="unstyled" id="listMedia">
                    ...
                    {{?(index+1) == item2.index}}
                    <li data-fdid="{{=item.id}}"><span class="title">{{=it.typeTxt}} <span class="index">{{=item2.index}}</span>：<span
                            class="name">{{=item.title}}</span></span>
