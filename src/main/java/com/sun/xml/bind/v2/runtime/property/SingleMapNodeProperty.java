...
final class SingleMapNodeProperty<BeanT,ValueT extends Map> extends PropertyImpl<BeanT> {
     ...
     *
     * The target will be set to a {@link java.util.Map}.
     */
    private final Loader itemsLoader = new Loader(false) {
        ...
        @Override
        public void startElement(State state, TagName ea) throws SAXException {
    ...
    private final Loader entryLoader = new Loader(false) {
        @Override
        public void startElement(State state, TagName ea) {
            state.target = new Object[2];  // this is inefficient
        }

        @Override
        public void leaveElement(State state, TagName ea) {
